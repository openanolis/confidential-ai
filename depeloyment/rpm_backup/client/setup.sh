...
set_secret() {
    ...
    for ((i=0; i<array_length; i++)); do
        ...
        if [ "$writer_type" = "trustee" ]; then
            ...
            local trustee_url=$(echo "$writer" | yq e '.params.kbs_address')
...
set_resource() {
    ...
    # Get all resource_writers from config
    local resource_writers=$(yq e '.resource_writer[]' "$config_file")
    
    # Iterate through each resource_writer
    echo "$resource_writers" | while read -r writer; do
        # Extract type
        local writer_type=$(echo "$writer" | yq e '.type')
