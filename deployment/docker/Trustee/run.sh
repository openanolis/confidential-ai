...
if [[ -e /dev/tdx_guest ]]; then
...
elif [[ -e /dev/csv_guest ]]; then
    echo "Detected: CSV guest environment (/dev/csv_guest present)"
    mkdir -p kbs/config/docker-compose
    ...
    echo "No /dev/tdx_guest or /dev/csv_guest found; not a TDX/CSV guest or drivers not loaded."
